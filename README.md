...
UWS, SoHo/Nolita, DUMBO) are drawn dashed as **considered-and-ruled-out**, each
with its stated reason (`data/ruledout.json`) — click them on the map. A
**Trains** toggle lazy-loads the real MTA subway routes (official line colors)
and stations from NY State Open Data (`public/subway-*.geojson`, regenerated by
`npm run fetch-geo`).
